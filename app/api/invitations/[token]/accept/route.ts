...
export async function POST(
  ...
  try {
    ...
    if (authError || !user) {
      return NextResponse.redirect(new URL(`/sign-in?message=Please sign in to accept this invitation&redirect_to=/invitation/${token}`, request.url));
    }
    ...
    if (error) {
      ...
      return NextResponse.redirect(new URL(`/invitation/${token}?error=${errorMessage}`, request.url));
    }

    if (data && data.success) {
      // Redirect to success page with organization context
      return NextResponse.redirect(new URL(`/invitation/${token}/success?redirect=true`, request.url));
    } else {
