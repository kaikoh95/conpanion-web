...
async function ProjectInvitationContent({ token, error }: { token: string; error?: string }) {
    ...
    return (
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <CardTitle className="text-red-600">Project Invitation Expired</CardTitle>
          <CardDescription>
        ...
        <CardContent className="text-center">
          <p className="text-sm text-muted-foreground">
            Please contact the project owner or admin for a new invitation.
          </p>
        </CardContent>
  ...
  if (!user) {
    redirect(`/sign-up?project-invitation=${token}`);
  }
  ...
  return (
    <Card className="w-full max-w-md">
      <CardHeader className="text-center">
        ...
        <CardDescription>
          This project invitation was sent to a different email address
        </CardDescription>
      </CardHeader>
      <CardContent className="text-center space-y-4">
        <div className="rounded-lg bg-amber-50 p-3">
          <p className="text-sm text-amber-800">
            <strong>Invitation email:</strong> {invitation.email}
          </p>
          <p className="text-sm text-amber-800">
        ...
        <p className="text-sm text-muted-foreground">
          Please sign in with the correct email address or contact the project administrator.
        </p>
      </CardContent>
      <CardFooter className="flex gap-3">
        ...
        <Button asChild className="flex-1">
          <Link href={`/sign-up?project-invitation=${token}`}>Sign Up with Correct Email</Link>
        </Button>
      </CardFooter>
    </Card>
  );
